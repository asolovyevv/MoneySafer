...
def populate(frame):
    ...
    Label(frame, text="Ваши операции", width=rootWidth, height=3, anchor=CENTER, bg='pink', bd=4).pack()
    ...
    def search():
        ...
        # cursor = conn.execute("SELECT * FROM Expense where edate='22/11/2019' or edate='20/11/2019' or
        # edate='21/11/2019' or edate='01/11/2019'")
        i = 0
        for r in cursor:
            if r[3] == 'expense':
                Label(dataFrame, text="%s" % (r[5]), width=20, height=3, borderwidth='1', relief='solid', bg=' salmon',
                      pady=3).grid(row=i, column=0)
                Label(dataFrame, text="%s - (%s) :" % (r[1], r[2]), width=50, height=3, borderwidth='1', relief='solid',
                      bg='salmon', pady=3).grid(row=i, column=1)
                Label(dataFrame, text='%s' % (r[4]), width=50, height=3, borderwidth='1', bg='salmon', relief='solid',
                      pady=3).grid(row=i, column=2)
                i += 1
            else:
                Label(dataFrame, text="%s" % (r[5]), width=20, height=3, borderwidth='1', relief='solid',
                      bg='light salmon', pady=3).grid(row=i, column=0)
                Label(dataFrame, text="%s - (%s) :" % (r[1], r[2]), width=50, height=3, borderwidth='1', relief='solid',
                      bg='light salmon', pady=3).grid(row=i, column=1)
                Label(dataFrame, text='%s' % (r[4]), width=50, height=3, borderwidth='1', bg='light salmon',
                      relief='solid', pady=3).grid(row=i, column=2)
                i += 1
        # Label(dataFrame, text = "%s - (%s) :"%(a,b),width=50,height = 3,borderwidth='1',relief = 'solid',
        # bg='green',pady=3).grid(row=i+1,column=0)

    Button(searchFrame, text="Поиск", width=10, command=search).grid(row=2, column=2)
